...
class Main(QDialog):
    ...
    def solve(self):
        ...
        if result == None:
            ...
            if sum_table < 100:  # Цикл-условие типа if "если сумма всех значений таблицы меньше 100"
                row = 1
                col = 0
                # замена значений второй строки нулями
                while col < self.tableWidget.columnCount():
                    zero = 0
                    self.tableWidget.setItem(row, col, QTableWidgetItem(str(zero)))
                    col += 1
                # перестановка min и max местами
